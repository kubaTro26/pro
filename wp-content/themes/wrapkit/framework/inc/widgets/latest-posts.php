 ...
 * 
 *
 * @description: A simple widget to display latest posts.
 * @package	WrapKit
 * @author Skat
...
class Wrapkit_Latest_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'sd_latest_posts_widget', // Base ID
			esc_html__( 'Latest Posts', 'wrapkit' ), // Name
			array( 'description' => esc_html__( 'Displays the latest posts.', 'wrapkit' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$sd_latest_posts = new WP_Query( $query_args );

		?>
		
		<div class="sd-latest-posts-widget">
			<?php if ( $sd_latest_posts->have_posts() ) : while ( $sd_latest_posts->have_posts() ) : $sd_latest_posts->the_post(); ?>
	...
	public function form( $instance ) {
		...
		$defaults = array(
			'title'         => esc_html__( 'Latest Posts', 'wrapkit' ),
			'postcount' => '2',
		);
...
function wrapkit_latest_posts_widget() {
    register_widget( 'Wrapkit_Latest_Posts_Widget' );
}
add_action( 'widgets_init', 'wrapkit_latest_posts_widget' );